...
def async_repeat_factory(count: int) -> Unary[T, AsyncIterator[T]]:
    def actual_async_repeat(value: T) -> AsyncIterator[T]:
        return async_repeat(value, count)

    return actual_async_repeat


def async_repeat_each(count: int, iterable: AnyIterable[T]) -> AsyncIterator[T]:
...
async def async_for_each_await(function: AsyncForEach[T], iterable: AnyIterable[T]) -> None:
    async for item in async_iter(iterable):
        await function(item)
...
def async_copy(iterable: AnyIterable[T], copies: int = 2) -> DynamicTuple[AsyncIterator[T]]:
    ...
    async def generator() -> AsyncIterator[T]:
        ...
        async for item in async_iter(state):
            yield item
...
def async_prepend(value: T, iterable: AnyIterable[T]) -> AsyncIterator[T]:
    return async_chain(async_once(value), iterable)


def async_append(value: T, iterable: AnyIterable[T]) -> AsyncIterator[T]:
    return async_chain(iterable, async_once(value))
...
async def async_group_simple(
    ...
    async def seek_group() -> AsyncIterator[Any]:
        ...
        return group(current_key, item, count)
...
async def async_pad(
    ...
    *,
    ...
    if size is None:
        async for item in async_chain(iterable, async_repeat(value)):
            yield item
...
async def async_filter_except(
    validate: Validate[T], iterable: AnyIterable[T], *errors: AnyErrorType
) -> AsyncIterator[T]:
    async for item in async_iter(iterable):
        try:
            validate(item)
...
async def async_filter_except_await(
    validate: AsyncValidate[T], iterable: AnyIterable[T], *errors: AnyErrorType
) -> AsyncIterator[T]:
    async for item in async_iter(iterable):
        try:
            await validate(item)
...
async def async_map_except(
    function: Unary[T, U], iterable: AnyIterable[T], *errors: AnyErrorType
) -> AsyncIterator[U]:
    async for item in async_iter(iterable):
        try:
            yield function(item)
...
async def async_map_except_await(
    function: AsyncUnary[T, U], iterable: AnyIterable[T], *errors: AnyErrorType
) -> AsyncIterator[U]:
    async for item in async_iter(iterable):
        try:
            yield await function(item)
...
async def async_iter_except(function: Nullary[T], *errors: AnyErrorType) -> AsyncIterator[T]:
    try:
        while True:
            yield function()
...
async def async_iter_except_await(
    function: AsyncNullary[T], *errors: AnyErrorType
) -> AsyncIterator[T]:
    try:
        while True:
            yield await function()
...
async def async_windows_with(
    function: Unary[Iterable[T], U], size: int, iterable: AnyIterable[T]
) -> AsyncIterator[U]:
    iterator = async_iter(iterable)
...
def async_list_windows(size: int, iterable: AnyIterable[T]) -> AsyncIterator[List[T]]:
    return async_windows_with(list, size, iterable)
...
def async_set_windows(size: int, iterable: AnyIterable[T]) -> AsyncIterator[Set[T]]:
    return async_windows_with(set, size, iterable)


async def async_inspect(function: Inspect[T], iterable: AnyIterable[T]) -> AsyncIterator[T]:
    async for item in async_iter(iterable):
        function(item)
        yield item


async def async_inspect_await(
    function: AsyncInspect[T], iterable: AnyIterable[T]
) -> AsyncIterator[T]:
    async for item in async_iter(iterable):
        await function(item)
        yield item
...
async def standard_async_map_await(
    function: DynamicAsyncCallable[R], *iterables: AnyIterable[Any]
) -> AsyncIterator[R]:
    async for args in async_zip(*iterables):
...
async def async_transpose(iterable: AnyIterable[AnyIterable[T]]) -> AsyncIterator[DynamicTuple[T]]:
    iterables = await async_extract(iterable)

    async for item in async_zip(*iterables):
        yield item


async def async_power_set(iterable: AnyIterable[T]) -> AsyncIterator[DynamicTuple[T]]:
    pool = await async_tuple(iterable)

    for count in inclusive(range(len(pool))):
        async for item in async_combinations(count, pool):
            yield item


async def async_permute(iterable: AnyIterable[T]) -> AsyncIterator[DynamicTuple[T]]:
    pool = await async_tuple(iterable)

    length = len(pool)

    for indices in cartesian_power(length, range(length)):
...
async def async_permutations(
    ...
    for indices in cartesian_power(count, range(length)):
...
async def async_combinations(
    ...
    for indices in permutations(count, range(length)):
...
def async_map_concurrent(function: AsyncUnary[T, U], iterable: AnyIterable[T]) -> AsyncIterator[U]:
